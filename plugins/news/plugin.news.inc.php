...
else
{
  ...
  $sql = "SELECT * FROM plugin_".$plugin->plugin_identify." WHERE plugin_news_delete_ts IS NULL ORDER BY plugin_news_create_ts DESC LIMIT ".$startval.", ".$endval.";";
	...
	foreach($new as $news)
	{
		...
		$commentcount = $db->get_var('SELECT COUNT(plugin_news_comment_id) FROM plugin_news_comments WHERE plugin_news_comment_news_id = "'.$news->plugin_news_id.'" AND plugin_news_comment_ts_delete IS NULL;');
		//$db->debug();
		
		$commentcount = ($commentcount == '0' || $commentcount == '')? 'keine':$commentcount;
		
		$news_content = explode('<!-- pagebreak -->', $news->plugin_news_content);
		$news_content_new = $news_content[0];
		//$news_content_new = $news->plugin_news_content;
